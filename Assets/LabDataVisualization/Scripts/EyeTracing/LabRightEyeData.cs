...
public class LabRightEyeData : LabDataBase
{
    /** 眼睛凝视点，基于右手坐标系*/
...
#if USE_SRANIPAL
    public LabRightEyeData(SingleEyeData data)
    {
    ...
    public LabRightEyeData(Pvr_UnitySDKAPI.EyeTrackingData data)
    {
        // 不確定串的值是不是對的
        gaze_origin_mm = data.rightEyeGazePoint.ToPos();
        gaze_direction_normalized = data.rightEyeGazeVector.ToPos();
        pupil_diameter_mm = data.rightEyePupilDilation;
        eye_openness = data.rightEyeOpenness;
        pupil_position_in_sensor_area = data.rightEyePositionGuide; 
    }
#endif
    public LabRightEyeData(
